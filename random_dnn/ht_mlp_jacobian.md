...
singular-value *magnitudes* to the singular-vector *localisation*: it drives
the cavity growth-rate criterion of `RMT/localisation.md` with this same
fixed-point profile and delivers the mobility edge $s^*(\alpha, \sigma_w)$
and the edge-truncated propagation gain $\chi_1^{<s^*}$. Section 7 lists
...
as the *localised* part $s > s^*$. The finite effective gain of the extended
(signal-propagating) modes is therefore the edge-truncated
$\chi_1^{<s^*} = \int_0^{s^*} s^2 f_{\mathrm{SV}}(s)\,ds$ (Section 6).
...
integral, as in `RMT/wishart_levy.py`). The Theorem 2
collapse does not
...
## 6. Singular-vector localisation: the mobility edge $s^*(\alpha, \sigma_w)$

Sections 1-5 give the singular-value *density* of $J^l$ -- the magnitude of
the stretching. The complementary question is whether the singular *vectors*
are delocalised (spread over $\Theta(N)$ coordinates) or localised
(concentrated on $o(N)$). For gradient propagation this is the difference
between a Jacobian that mixes directions and one that routes signal through a
handful of neurons. (We write the singular-value mobility edge as $s^*$ --
the critical singular value, the Anderson $E_c$ analogue.)

**The criterion.** The transpose $J^{l\mathsf t} = W^{l\mathsf t} D^l$ is the
**row-profile** specialisation $a_i = |\phi'(h^l_i)|$ of the structured
heavy-tailed ensemble of `RMT/localisation.md`. Localisation is governed
there by the stability of the $\mathrm{Im}\,G = 0$ fixed point of the
bipartite cavity recursion, measured directly by population dynamics: the
growth rate $\varphi(s)$ of the mean log imaginary part of the resolvent
under the linearised cavity recursion (the quenched Lyapunov exponent).
$\varphi(s) > 0$ is delocalised, $\varphi(s) < 0$ localised; the mobility
edge $s^*$ is the zero of $\varphi$. Finite population-dynamics pools bias
$\varphi$ downward, so each $s$-point runs a pool-doubling ladder with
Brunet-Derrida extrapolation $\varphi_P = \varphi_\infty - c/\ln^2 P$:
any fixed-$P$ edge is a lower bound (precision product), the extrapolated
edge is the accuracy product. Solver: `RMT/localisation.py`; sweep driver:
`.agents/scripts/phi_star_sweep.py`; production artifact:
`fig/ht_mlp_jacobian/phi_star_phase_diagram;rungs=2500-20000;K=256;n_s=14.npz`.

**The profile is the same fixed-point object as the density (with
$\sigma_w$).** The physical Jacobian entry is
$J_{ij} = |\phi'(h^l_i)|\,\sigma_w N^{-1/\alpha}\tilde W_{ij}$, so the
cavity row profile carries the global scale:
$a_i = \sigma_w\,|\phi'((q^*)^{1/\alpha} Z_i)|$, with $Z_i \sim p_\alpha$
and $(q^*)^{1/\alpha}$ the preactivation scale at the fixed point of
`heavy_tailed_mlp.md`. The single map $(\alpha, \sigma_w) \mapsto q^\star
\mapsto \{|\phi'((q^*)^{1/\alpha} Z_i)|\}$ thus drives *both* the SV
density (Sections 3-5) and the SV localisation (this section). The cavity
amplitudes use the Chambers-Mallows-Stuck physical convention, so the
estimator returns $s^*$ directly in physical singular-value units.
...
profile $|\phi'| \le 1$ does not change the entry tail index $\alpha$, so
it cannot move that baseline by reweighting alone. The mechanism that *does*
localise is **profile sparsification**: at large $\sigma_w$ the fixed-point
scale $(q^*)^{1/\alpha}$ is large, a finite fraction of preactivations land
in the saturated tail of $\tanh$ where $\phi' \approx 0$, and those rows
are effectively cut from the connectivity graph. Below a critical saturated
fraction the spectrum stays delocalised; above it a genuine localised tail
opens. The saturated fraction is monotone in $\sigma_w$ through $q^\star$
(e.g. at $\alpha = 1.5$: $\sigma_w = 1 \Rightarrow q^\star = 0.11$,
sat $= 0.01$; $\sigma_w = 3 \Rightarrow q^\star = 3.6$, sat $= 0.33$).

**Result.** The production map (330 cells, alpha 1.0-2.0 x sigma_W 0.1-3.0,
all finite) is delocalised at small $\sigma_w$ -- consistent with the BG
baseline -- with a localised tail opening as saturation grows; at the anchor
$(\alpha, \sigma_w) = (1.5, 1.0)$ the top-rung ($P = 2 \times 10^4$) edge
is $s^* \approx 4.7$ and the BD-extrapolated edge $\approx 5.9$-$6.1$
(direct mean-log readout 4.67 / 6.09, retired frozen-moment readout
4.67-4.71 / $5.9 \pm 0.5$ -- the two coincide in the pool limit).
The $\alpha = 2$ row is the Gaussian support edge, not a localisation
transition. Validation is against direct diagonalisation: IPR / $D_q$ of
exact singular vectors across an $N$-ladder
(`mlp_jacobian_Dq_spectrum`, `synthetic_jacobian_Dq_spectrum`), with the
small-$\sigma_w$ baseline reproducing BG delocalisation.

**The mobility edge regularises the propagation gain.** The per-layer gain
that governs how a perturbation grows is the mean-square singular value
$\chi_1 = \langle s^2\rangle = \int s^2 f_{\mathrm{SV}}(s)\,ds$ (Poole et
al.'s $\chi_1$ at $\alpha = 2$). For the heavy-tailed Jacobian this
**diverges**: the SV density tail $f_{\mathrm{SV}}(s) \sim
B\,s^{-(1+\alpha)}$ makes the integrand $\sim B\,s^{1-\alpha}$
non-integrable for $\alpha < 2$. The divergence is carried entirely by the
heavy SV tail, which is exactly the **localised** part $s > s^*$ -- modes
that do not propagate coherent signal across the layer. The finite gain of
the extended (signal-carrying) modes is therefore the **edge-truncated**

$$
\chi_1^{<s^*} \;=\; \int_0^{s^*} s^2\, f_{\mathrm{SV}}(s)\,ds
\;=\; \tfrac1N \!\!\sum_{k:\,s_k < s^*}\!\! s_k^2 ,
$$

with the mobility edge supplying the natural cutoff. It is $N$-stable and
finite on the localised grid, and its unit contour
$\chi_1^{<s^*} = 1$ is the delocalised edge of chaos -- the results
narrative, including the finite-$N$ band structure, is
`deloc_edge_of_chaos.md`. The repo carries both versions of the quantity:

- `truncated_chi1` -- empirical SVD estimate below a supplied cutoff
- `theoretical_truncated_chi1` -- analytical value from the SV density at a
  supplied cutoff; the production join of density and MC edges at all
  rung/BD cutoffs is `.agents/scripts/chi1_mc_recut.py`
  (artifact `phi_star_truncated_chi1;...npz`)
