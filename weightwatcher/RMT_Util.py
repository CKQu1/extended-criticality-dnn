...
def save_fig(plt, figname, layer_id, savedir):
    ...
    figname = "{}/ww.{}.{}.png".format(savedir, layer_id, figname)
...
def fit_clipped_powerlaw(evals, xmin=None, xmax=None, total_is=1000, verbose=False, max_fingers=DEFAULT_MAX_FINGERS, min_alpha=2.0, 
    ...
    if xmin is not None and xmin != -1:
        ...
        prev_fit =  pl_fit(data=evals, xmin=xmin, xmax=xmax, total_is=total_is, verbose=verbose, distribution=POWER_LAW, pl_package=pl_package)
    else:
        #prev_fit = powerlaw.Fit(evals, xmax=xmax)
        prev_fit =  pl_fit(data=evals, xmax=xmax, total_is=total_is, verbose=verbose, distribution=POWER_LAW, pl_package=pl_package)
    ...
    for idx in range(2,max_fingers):
        ...
        if xmin is not None and xmin != -1:
            ...
            fit = pl_fit(data=evals[:-idx], xmin=xmin, xmax=xmax, total_is=total_is, verbose=verbose, distribution=POWER_LAW, pl_package=pl_package)
        else:
            #fit = powerlaw.Fit(evals, xmax=xmax, verbose=verbose)
            fit = pl_fit(data=evals[:-idx], xmax=xmax, total_is=total_is, verbose=verbose, distribution=POWER_LAW, pl_package=pl_package)
    ...
    #  provide the next 3 alpha, and issue a warning if they are out of whack
        ...
        for idy in range(idx+1, idx+4):
            ...
            # if xmax==-1 ?
            if xmin is not None and xmin != -1:
                check_fit = pl_fit(data=data, xmin=xmin, xmax=xmax, total_is=total_is, verbose=verbose, distribution=POWER_LAW, pl_package=pl_package)
            else:
                check_fit = pl_fit(data=data, xmax=xmax, total_is=total_is, verbose=verbose, distribution=POWER_LAW, pl_package=pl_package)
