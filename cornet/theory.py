...
"""
def integrand(z):
    return norm.pdf(z[:, None]) * np.tanh(np.sqrt(qin) * z[:, None])**2
"""
...
def q_fixed_point(weight_sigma=1, bias_sigma=0, nonlinearity=np.tanh, max_iter=500, tol=1e-9, qinit=3.0, fast=True, tol_frac=0.01):
