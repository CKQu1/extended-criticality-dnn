...
for i in range(len(axs)):
    ...
    #axs[i].set_ylim(0.205 - ch,3.045 + ch)
    ...
    axs[i].tick_params(axis="x", which='minor', direction="in")
    axs[i].tick_params(axis="y", which='minor', direction="in", length=0.25)

# --------------------------------------------------------------------

alpha_m_ls = []
#cmap_bd = [[0,np.log(2)]] + [[0,np.log(70)]]*3
#cmap_bd = [[0,1]] + [[0,10], [0,25], [0,30]]        #logged
#cmap_bd = [[0,30]] + [[0,120], [0,120], [0,150]]     #std/mean no log
#cmap_bd = [[0,1.5], [0,500], [0,5e10], [0,0.5e13]]  # no logged

cmap_bd = [[0,5]] + [[0,11], [0,14], [0,15]]
...
for metric_idx in range(len(metric_names)):
    metric_data = np.empty((len(layer_ls), mult_N * alpha_N))
    #metric_data = torch.empty((len(layer_ls), mult_N * alpha_N))
    metric_name = metric_names[metric_idx]
    #metric_name_std = metric_names_std[metric_idx]
    grid_idx = 0
    for alpha_idx in range(len(alpha_grid)):
        for m_idx in range(len(mult_grid)):
            ...
            _, w_alpha, w_mult, N_0, q, _ = init_params.iloc[0,:]    
            N_0 = int(N_0)    
            alpha_m_ls.append((w_alpha,w_mult))            

            # average the ensembles (coarse-graining)
            data_raw = torch.load(f"{net_path_full}/{metric_name}_{round(alpha,1)}_{round(mult,2)}")
            #data_raw = torch.load(f"{net_path_full}/{metric_name}")
            #data_raw_std = torch.load(f"{net_path_full}/{metric_name_std}")  
            #metric_data[:,grid_idx] = data_raw[layer_ls,:].T/N_0
            #metric_data[:,grid_idx] = data_raw[layer_ls,:].T/(N_0 * q ** torch.tensor(layer_ls).T) # scale curvature down by q
            #metric_data[:,grid_idx] = data_raw_std[layer_ls,:].T/data_raw[layer_ls,:].T # scale curvature down by q
            metric_data[:,grid_idx] = np.nanmean(data_raw.numpy(), axis=0)[layer_ls]
    ...
    for layer_idx in range(len(layer_ls)):
        ...
        for t in range(len(alpha_m_ls)):
            ...
            #metric_mesh[x_loc,y_loc] = np.log(metric_layer[t])
        ...
        # delete
        print(f"{layer_id}")
        print((np.min(metric_mesh), np.max(metric_mesh)))
        print("\n")
