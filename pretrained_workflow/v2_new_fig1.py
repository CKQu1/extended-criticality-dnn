...
cm_type_2 = 'RdGy'
c_ls_1 = ['k', 'k']
c_ls_2 = ["tab:blue", "tab:red", "tab:green", "tab:purple", "tab:orange", "tab:green", "tab:brown", "tab:olive"]
...
if not (isfile(join(grouped_stats_path, df_full_file)) and isfile(join(grouped_stats_path, df_tail_file))) or replace:
    ...
    for ii in tqdm(range(len(metrics_all['param_shape']))):
        ...
        if fit_done:
            ...
            # Best fit                             
            bf1_lower = df_pl.loc[0,'bestfit']; bf1_upper = df_pl.loc[1,'bestfit']

            # Moments
            skewness, kurtosis = df_pl.loc[0,'skewness'], df_pl.loc[0,'kurtosis']
        ...
        for prefix in ['alpha', 'tpl_alpha', 'bf1', 'xmin', 'xmax', 'xlogrange']:
            for tail_type in ['lower', 'upper']:
                metrics_tail[f'{prefix}_{tail_type}'].append( locals()[f'{prefix}_{tail_type}'] )

        for name_ in ['skewness', 'kurtosis']:
            metrics_tail[name_].append( locals()[name_] )
...
metric_names_plot = ["sample_w", "sample_w", "alpha", "alpha_tail_tpl", "kurtosis", "alpha_tail_pl", "nu", "xlogrange"]
xlim_ls = [[-0.25, 0.25], [1e-2,1], [0.5,2.1], [0.5,8], [-2, 30], [0,10], [0,18], [0,4]]
ylim_ls = [[0,14], [1e-2, 100], [0,2], [0,0.4], [0,0.15], [0,1], [0,0.2], [0,1.75]]
...
for i, metric_name in enumerate(metric_names_plot):
    ...
    if i == 0:
        axis.hist(sample_w[np.abs(sample_w) > 1e-5], bin_ls[i], color=c_ls_2[color_ii], density=True)       
        ...
        if metric_name in ["alpha_tail_pl", "alpha", "xlogrange"]:
