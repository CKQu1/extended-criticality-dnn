...
#fig = figure(figsize=(12.5,2*3.5 + 2.5)) #2 by 3
fig, axs = plt.subplots(3, 3, sharex = False,sharey=False,figsize=(12.5 + 4.5, 9.5/2*3 + 0.5))
...
for i in range(3):
    ...
    if i == 0:
        ...
        cbar.ax.set_xticklabels(cbar_ticks,size=tick_size-3)
...
# plot accuracy
fcn_axs[2].plot(acc_loss['training_history'][:,1], c=c_ls_3[0], linewidth=1.5*1.8, label="Train")
fcn_axs[2].plot(acc_loss['testing_history'][:,1], c=c_ls_3[1], linewidth=1.5*1.8, linestyle="--", label="Test")
fcn_axs[2].legend(bbox_to_anchor=(0.9,0.9), fontsize = legend_size, frameon=False)
...
image_type = 'mnist'
batch_size = 1000
trainloader, _ = set_data(image_type, rshape=False)
images = trainloader.train_data
image = images[0,:,:].float()
# leave out zers for aesthetics
image[image==0] = np.NaN
colmap = plt.cm.get_cmap(cm_type_2)
colmap.set_bad(color="white")
axs[2,0].imshow(image, aspect="auto", cmap=colmap)
axs[2,0].set_title("Input image")

# plot settings
for col in range(3):
    xyticks = np.array([0,13,27])
    axs[2,col].set_xticks(xyticks)
    axs[2,col].set_yticks(xyticks)
    axs[2,col].set_xticklabels(xyticks+1)
    axs[2,col].set_yticklabels(xyticks[::-1]+1)
...
plt.savefig(f"{fig1_path}/pretrained_stablefit_torch8.pdf", bbox_inches='tight')
