...
from UTILS.data_utils import set_data, get_data
...
def train_ht_dnn(name, Y_classes, X_clusters, cluster_seed, assignment_and_noise_seed, num_train, num_test,
    ...
    if name.lower() == "gaussian":
        ...
        if not os.path.isfile(njoin(root_path, "gaussian_data_setting.csv")):
            gaussian_data_setting = pd.DataFrame(gaussian_data_kwargs, index=[0], dtype=object)
            gaussian_data_setting.to_csv(njoin(root_path, "gaussian_data_setting.csv"), index=False)
        # save centers and labels for gaussian data (no longer needed)
        """
        if not os.path.isfile(njoin(root_path, f"centers.npy")):
            np.save(njoin(root_path, "centers"), centers)
        if not os.path.isfile(njoin(root_path, f"cluster_class_label.npy")):
            np.save(njoin(root_path, "cluster_class_label"), cluster_class_label)
        """
...
def train_ht_cnn(root_path, name, alpha100, g100, optimizer, net_type, fc_init, lr=0.001, activation="tanh",                                             # network structure
    ...
    # create path
    #model_path = njoin(DROOT, "trained_cnns", net_type + "_new", f"{net_type}_{alpha100}_{g100}_{model_id}_{name}_{optimizer}_lr={lr}_bs={bs}_epochs={epochs}")
    model_path = njoin(root_path, net_type, f"{net_type}_{alpha100}_{g100}_{model_id}_{name}_{optimizer}_lr={lr}_bs={bs}_epochs={epochs}")
    if not os.path.isdir(model_path): os.makedirs(f'{model_path}')
