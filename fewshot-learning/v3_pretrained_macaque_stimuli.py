...
from pretrained_macaque_stimuli import get_CI, transform_model_name
...
def load_shared_info(model_name):
    assert os.path.isfile(join(root_data, "featured_macaque_stimuli", model_name, "shared_info")), f'shared_info does not exist for {model_name}'
    return pd.read_csv(join(root_data, "featured_macaque_stimuli", model_name, "shared_info"))  
...
def load_dict():
    metric_dict = {"SNR"        : "SNRs_layerwise",
                   ...
                   'r_rshpsingvec_mean': 'r_rshpsingvec_dq',
                   'l_rshpsingvec_mean': 'l_rshpsingvec_dq',
                   "r_rshpsingvec_principal": "r_rshpsingvec_dq",
                   "l_rshpsingvec_principal": "l_rshpsingvec_dq"                                       
                   }
    ...
    name_dict = {"SNR"        : "SNR",
                 ...
                 'r_rshpsingvec_mean': 'Right rshp singvec (mean)',
                 'l_rshpsingvec_mean': 'Left rshp singvec (mean)',
                 'r_rshpsingvec_principal': 'Right rshp singvec (max)',
                 'l_rshpsingvec_principal': 'Left rshp singvec (max)'                                               
                 }   
...
def load_raw_metric(model_name, pretrained:bool, metric_name, **kwargs):
    ...
    global metric_lidx, metric_data, df, weight_shape, has_weight, metric_lidx_shape
    ...
    # model path
    init_path = join(root_data, "featured_macaque_stimuli")
...
def snr_metric_plot(main=True, n_top=100, display=False):
    global metric_data_all, metric_data, d2_data, metric_data_y, singvec_data, total_layers, lidxs, df, weight_shapes, d2_name, d2_data_mean
    global selected_layers, selected_layer   
    ...
    metric_names_options = ["d2_avg,D,SNR,error", "hidden_d2,D,SNR,error",
                            "r_singvec_mean,D,SNR,error", "l_singvec_mean,D,SNR,error",
                            "r_singvec_principal,D,SNR,error", "l_singvec_principal,D,SNR,error",
                            "r_fftsingvec_mean,D,SNR,error", "l_fftsingvec_mean,D,SNR,error",
                            "r_fftsingvec_principal,D,SNR,error", "l_fftsingvec_principal,D,SNR,error",
                            "r_rshpsingvec_mean,D,SNR,error", "l_rshpsingvec_mean,D,SNR,error",
                            "r_rshpsingvec_principal,D,SNR,error", "l_rshpsingvec_principal,D,SNR,error"]
    ...
    for metric_names in metric_names_options:        
        ...
        for pretrained_idx, pretrained in enumerate(pretrained_ls):
            ...
            for nidx in range(len(model_names)):
                ...
                # load shared_info
                if pretrained_idx == 0:  # only load once for the same net type
                    df = load_shared_info(model_names[nidx])    
                    lidxs = []  # these are the layers that are either Conv2d or FC
                    weight_shapes = []
                    for lidx in range(df.shape[0]):
                        has_weight = df.loc[lidx,'has_weight']
                        has_weight = bool(has_weight == np.bool_(True))   
                ...
                #total_layers = len(load_processed_metric(model_name, pretrained, "SNR", m=1))
                total_layers = len(load_processed_metric(model_name, pretrained, "SNR", m=1))
                ...
                for metric_idx, metric_name in enumerate(metric_names):
                    ...
                    if 'singvec' not in metric_name or metric_name == 'hidden_d2':
                        ...
                        for metric_lidx in metric_data:                        
                            if 'mean' in metric_name:
                                ...
                                if metric_lidx.ndim == 1:
                                    ...
                                    metric_data_std.append(np.nan)
                                else:
                                    metric_data_mean.append(metric_lidx[:,:,0].mean())
                                    metric_data_std.append(metric_lidx[:,:,0].std())                                          
                        
                    #print(metric_data.shape)
                    # frac_layers or lidxs
        ...
        if not display:
            if main in [True, False]:
                ...
                fig_path = join(root_data,f"figure_ms/pretrained-fewshot-v3_conv_fc_layers")
