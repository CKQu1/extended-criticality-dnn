...
from wmat_fcn import get_wmats

"""
Plots the principal eigenvector lozalization sites
"""
def eigvec_magnitude(reig=1, is_eig=True, inset=False, alpha100_ls = [120,200], g100 = 100):
    reig = int(reig)
    is_eig = literal_eval(is_eig) if isinstance(is_eig, str) else is_eig    
    inset = literal_eval(inset) if isinstance(inset, str) else inset
    vec_type = "eig" if is_eig else "sing"
    #print(f"Computations for {vec_type}vectors")
        ...
        for f_idx in range(len(alpha100_ls)):
            #for f_idx in range(len(extension_names)):
                ...
                for layer in [4]:
                    ...
                    wmat = wmats[layer].detach().numpy()

                    # left eigenvector
                    if reig == 0:
                        wmat = wmat.T

                    # this is also documented in dq_analysis/wmat_fcn.py
                    if wmat.shape[0] != wmat.shape[1] or not is_eig:
                        _, eigvals, eigvecs = la.svd(wmat)
                    else:
                        eigvals, eigvecs = la.eig(wmat)                             

                    print(f"Epoch {epoch} layer {layer}: {wmat.shape}")
                    print(f"Max {vec_type}value magnitude: {np.max(np.abs(eigvals))}.")
                    print(f"Max {vec_type}vector site: {np.max(np.abs(eigvecs[:,np.argmax(np.abs(eigvals))]))}")
                    ...
                    fig1_path = join(root_data, f"figure_ms/dq_wmat_{reig_dict[reig]}_plots")
                    if not os.path.isdir(fig1_path): os.makedirs(fig1_path)
                    # alleviate memory
                    fig_name = f"wmat_{vec_type}vec_mag_{reig_dict[reig]}_alpha100={alpha100}_g100={g100}_l={layer}_epoch={epoch}"
                    if inset:
                        fig_name += "_inset"
...
def dq_vs_q(reig=1, is_eig=True, alpha100_ls = [120,200], g100 = 100):
    reig = int(reig)
    is_eig = literal_eval(is_eig) if isinstance(is_eig, str) else is_eig
    vec_type = "eig" if is_eig else "sing"
    ...
    dq_path = join(root_data, f"geometry_data/wmat_dq_{reig_dict[reig]}")
    ...
    for epoch in [0,650]:
        for layer in range(0,10):
            ...
            for f_idx, alpha100 in enumerate(alpha100_ls):
                extension_name = f"alpha{alpha100}_g{g100}_ep{epoch}.txt"

                df_mean = np.loadtxt(f"{dq_path}/wmat_{vec_type}vec_dqmean_{extension_name}")
                df_std = np.loadtxt(f"{dq_path}/wmat_{vec_type}vec_dqstd_{extension_name}")
                qs = df_mean[:,0]
            ...
            if not os.path.isdir(fig1_path): os.makedirs(fig1_path)
            # alleviate memory
            plt.savefig(f"{fig1_path}/dq_wmat_{vec_type}vec_{reig_dict[reig]}_l={layer}_epoch={epoch}.pdf", bbox_inches='tight')
            plt.clf()
            plt.close(fig)
...
def d2_vs_depth(reig=1, is_eig=False, appendix=False, alpha100_ls=[120,200], g100=300):
    ...
    for epoch in [0, 650]:
        ...
        if not os.path.isdir(fig1_path): os.makedirs(fig1_path)
        # alleviate memory
        plt.savefig(f"{fig1_path}/wmat_{vec_type}vec_d2-vs-depth_{reig_dict[reig]}_g100={g100}_epoch={epoch}.pdf", bbox_inches='tight')
        #plt.clf()
        #plt.close(fig)
        #plt.show()
...
def d2_vs_eigvals(alpha100_ls = [120,200], g100 = 100, reig=1, is_eig=True):
    ...
    data_path = join(root_data, f"geometry_data/wmat_layerwise")
    ...
    for epoch in [0,650]:
        ...
        for f_idx, alpha100 in enumerate(alpha100_ls):
                ...
                for layer in [4]:
                    ...
                    wmat = wmats[layer].numpy()
                    # left eigenvector
                    if reig == 0:
                        wmat = wmat.T

                    print(f"layer {layer}: {wmat.shape}")
                    eigvals, eigvecs = la.eig(wmat)
        ...
        if not os.path.isdir(fig1_path): os.makedirs(fig1_path)
        # alleviate memory
        plt.savefig(f"{fig1_path}/wmat_{vec_type}vec_d2-vs-eigval_{reig_dict[reig]}_alpha100={alpha100}_g100={g100}_l={layer}_epoch={epoch}.pdf", bbox_inches='tight')
        plt.clf()
        plt.close(fig)
